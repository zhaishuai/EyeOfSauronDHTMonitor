...
//        });
        ...
        try {
            ...
            sender.connect("180.149.132.47", 80, [&sender](uv_stream_t *stream){
                for(int i = 0 ; i < 1 ; i++){
                    sender.send(stream, "GET /hello HTTP/1.1\r\n"
                                "Host: 0.0.0.0=8000\r\n"
...
#define LOGF(...)
    ...
    void on_resolved(uv_getaddrinfo_t *req, int status, struct addrinfo *res) {
        ...
        int r = uv_ip4_addr("10.12.112.55", 6882, &dest);
        CHECK(r, "ip4_addr");
    ...
    void main3() {
        ...
        int r = uv_getaddrinfo(uv_loop, &req, on_resolved, "127.0.0.1", "6882", &hints);
